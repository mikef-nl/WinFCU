...
namespace Total.WinFCU
{
    public partial class fcu
    {
        ...
        private static string[] GetAllFilesFromFolder(string root, SearchOption searchSubfolders, bool followSymLinks, string filename = "*.*")
        {
            ...
            while (folders.Count != 0)
            {
                ...
                try
                {
                    ...
                    {
                        if (!followSymLinks & fcu.IsSymbolicLink(folder)) { total.Logger.Debug("Skipping symlink folder \"" + folder + "\""); continue; }
        ...
        public static void LoadConfiguration(string fcuConfigFile)
        {
            ...
            defAttributes.followSymLinks     = false;
            ...
            try
            {
                foreach (XmlAttribute attribute in fcu.fcuConfig.SelectSingleNode(defQuery).Attributes)
                {
                    ...
                    switch (defValue.ToLower())
                    {
                        ...
                        case "followsymlinks":     defAttributes.followSymLinks     = Convert.ToBoolean(attribute.Value); break;
                ...
                foreach (XmlElement incNode in fcu.fcuConfig.SelectNodes(incQuery))
                {
                    ...
                    // Process all files found
                    {
                        ...
                        foreach (FileInfo incFile in incFileInfo)
                        {
                            ...
                            string incSystem = "";
                            try   {
                                foreach (XmlNode incApp in incAppList)
                                {
                                    if (incApp.Attributes.Count == 0) { continue; }
                                    incSystem = incApp.Attributes.GetNamedItem("system").Value;
                                }
                            }
...
*/
            ...
            // --------------------------------------------------------------------------------------------------------------------
            defineSpecialActions();
            total.Logger.Debug("Loading Schedule definitions");
            string spcQuery = String.Format(xmlTransNode, "configuration") +
                              String.Format(xmlTransNode, "applicationsettings") +
                              String.Format(xmlTransNode, "specials") +
                              String.Format(xmlTransNode, "special");
            try
            {
                foreach (XmlElement spcNode in fcu.fcuConfig.SelectNodes(spcQuery))
                {
                    string spcName     = "";
                    bool   spcEnabled  = false;
                    string spcSystem   = "";
                    string spcSchedule = "";
                    foreach (XmlAttribute spcAttribute in spcNode.Attributes)
                    {
                        switch (spcAttribute.Name.ToLower())
                        {
                            case "name":        spcName = spcAttribute.Value; break;
                            case "enabled":     spcEnabled = (spcAttribute.Value == "true"); break;
                            case "system":      spcSystem = spcAttribute.Value; break;
                            case "schedule":    spcSchedule = spcAttribute.Value; break;
                            default: total.Logger.Error("Invalid specials attribute \"" + spcAttribute.Name + "\" found in " + spcNode.OuterXml); break;
                        }
                    }
                    specialsInfo siObject = specialActions[spcName];
                    siObject.enabled  = spcEnabled;
                    siObject.systemOk = String.IsNullOrEmpty(spcSystem) || Regex.Match(total.ENV.Server, spcSystem, RegexOptions.IgnoreCase).Success;
                    siObject.schedule = spcSchedule;
                    specialActions[spcName] = siObject;
                    if (spcEnabled && siObject.systemOk) { runHasSpecials = true; }
                }
            }
            catch (Exception ex) { total.Logger.Fatal("Error processing Specials settings.\n", ex); Environment.Exit(1); }
            // --------------------------------------------------------------------------------------------------------------------
...
*/
        ...
        public static void CleanFileSystem(string scheduleName = "#ALL#")
        {
            ...
            foreach (XmlElement folderSet in fcu.fcuFolders)
            {
                ...
                foreach (XmlAttribute attribute in folderSet.Attributes)
                {
                    switch (attribute.Name.ToLower())
                    {
                        ...
                        case "followsymlinks":     folderSetAttr.followSymLinks     = Convert.ToBoolean(folderSet.GetAttribute(attribute.Name)); break;
                ...
                foreach (var folderElement in folderSet)
                {
                    ...
                    foreach (XmlAttribute attribute in fcuFolder.Attributes)
                    {
                        switch (attribute.Name.ToLower())
                        {
                            ...
                            case "followsymlinks":     folderAttr.followSymLinks     = Convert.ToBoolean(fcuFolder.GetAttribute(attribute.Name)); break;
                    ...
                    scanPath = ReplaceKeyword(scanPath);
                    total.Logger.Info("Start scan on: " + scanPath);
                    // ------------------------------------------------------------------------------------------------------------
                    ...
                    foreach (var fileElement in fcuFolder)
                    {
                        ...
                        foreach (XmlAttribute attribute in fileDef.Attributes)
                        {
                            switch (attribute.Name.ToLower())
                            {
                                ...
                                case "followsymlinks":     fileAttr.followSymLinks     = Convert.ToBoolean(fileDef.GetAttribute(attribute.Name)); break;
                        ...
                        foreach (string filePath in pathList)
                        {
                            ...
                            if (!folderAttr.followSymLinks & fcu.IsSymbolicLink(filePath)) { total.Logger.Debug("Skipping symlink folder \"" + filePath + "\""); continue; }
                            ...
                            {
                                total.Logger.Info(" scanning: " + filePath + " for: " + fileName);
                                allFiles.AddRange(GetAllFilesFromFolder(filePath, recursiveScan, fileAttr.followSymLinks, fileName));
                            }
                            // ----------------------------------------------------------------------------------------------------
                            ...
                            foreach (string fcuFile in allFiles.Distinct())
                            {
                                ...
                                if (!fileAttr.followSymLinks & fcu.IsSymbolicLink(fcuFile)) { total.Logger.Debug("Skipping symlink file \"" + fcuFile + "\""); continue; }
                    ...
                    // ------------------------------------------------------------------------------------------------------------
                    //   Finally: delete empty subfolders?? Lets do so now
                    // ------------------------------------------------------------------------------------------------------------
                    if (folderAttr.deleteEmptyFolders) { fcu.DeleteEmptyFolders(scanPath, new Regex(@"DoNotExcludeAny"), false, SearchOption.AllDirectories); }
                    // ------------------------------------------------------------------------------------------------------------
                }
                // ----------------------------------------------------------------------------------------------------------------
            ...
            // --------------------------------------------------------------------------------------------------------------------
            // Any specials to process in this schedule?
            // --------------------------------------------------------------------------------------------------------------------
            total.Logger.Info("Checking for WinFCU special actions");
            if (fcu.runHasSpecials) { fcu.processSpecialActions(scheduleName); }
            // --------------------------------------------------------------------------------------------------------------------
            //   Finish the job by showing how much space has been regained
            // --------------------------------------------------------------------------------------------------------------------
            if (fcu.EventLogInitialized && !total.APP.Dryrun)
            {
                string evtLogMessage = "WinFCU filesystem cleanup run completed";
                evtLogMessage += String.Format(" - Total bytes deleted  : {0}\n", fcu.total_bytesDeleted);
                evtLogMessage += String.Format(" - Total bytes archived : {0}\n", fcu.total_bytesArchived);
                evtLogMessage += String.Format(" - Total bytes moved    : {0}\n", fcu.total_bytesMoved);
                evtLogMessage += String.Format(" - Total bytes compacted: {0}\n", fcu.total_bytesCompacted);
                fcu.evtLog.WriteEntry(evtLogMessage, EventLogEntryType.Information, 0);
            }

            string prfx = total.APP.Dryrun ? " [DRYRUN] - " : "";
            total.Logger.Info(dash80);
            total.Logger.Info(prfx + "Total bytes deleted  : " + fcu.total_bytesDeleted);
            total.Logger.Info(prfx + "Total bytes archived : " + fcu.total_bytesArchived);
            total.Logger.Info(prfx + "Total bytes moved    : " + fcu.total_bytesMoved);
            total.Logger.Info(prfx + "Total bytes compacted: " + fcu.total_bytesCompacted);
            total.Logger.Info(dash80);
            // --------------------------------------------------------------------------------------------------------------------
        ...
        public static void EmptyRecycleBins()
        {
            ...
            try
            {
                ...
                {
                    foreach (string rbFile in GetAllFilesFromFolder(rbPath, SearchOption.AllDirectories, false, "*")) { File.Delete(rbFile); }
                }
