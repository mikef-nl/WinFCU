...
namespace Total.WinFCU
{
    public partial class fcu
    {
        ...
        // ------------------------------------------------------------------------------------------------------------------------
        public static void DeleteEmptyFolders(string rootPath, Regex excludeFolder, bool followSymLinks, SearchOption recursiveScan)
        {
            string[] allFolders = null;
            total.Logger.Info("Removing empty folders in " + rootPath);
            // --------------------------------------------------------------------------------------------------------------------
            //  Create a list of available folders. The scan path might contain wildcards, resolve the path and scan folder by folder
            // --------------------------------------------------------------------------------------------------------------------
            string[] pathList = total.ResolvePath(rootPath);
            foreach (string scanPath in pathList)
            {
                ...
                {
                    if (excludeFolder.Match(pathName).Success) { total.Logger.Debug("Excluding path: " + pathName); continue; }
                    if (!followSymLinks & fcu.IsSymbolicLink(pathName)) { total.Logger.Debug("Skipping symlink folder \"" + pathName + "\""); continue; }
                    ...
                    else
                    {
                        ...
                        if (dirDeleted) { total.Logger.Info("Removed folder " + pathName); }
                    }
                }
